...
export const onExecute = async ({ Records }: SQSEvent) => {
  ...
  for (const record of Records) {
    const { body } = record
    const { vaultId, side, amount, tokenId } = JSON.parse(body) as IExecuteQueue
    ...
    const encodefunc = stoxContract.interface.encodeFunctionData('execute', [
      {
        order: {
          side,
          maker: maker[0].address,
          tokenId,
          amount,
